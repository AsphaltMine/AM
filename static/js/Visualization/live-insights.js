...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function setupRefreshButton() {
        ...
        btn.addEventListener('click', function () {
            ...
            var usesCache = state.scope === 'all';
            var forceRefresh = usesCache
                ? fetch('/visu/live-insights/refresh/', { method: 'POST', credentials: 'same-origin' })
                : Promise.resolve();
            forceRefresh
                .then(function () { return refreshAll(); })
                .catch(function () { return refreshAll(); })
                .then(function () { btn.classList.remove('li-refresh-spinning'); });
        });
    }
    ...
    function markUpdated(serverTimestamp) {
        lastUpdated = serverTimestamp ? new Date(serverTimestamp) : new Date();
        renderUpdatedText();
    }
    ...
    function openCorrelationScatter(fieldA, fieldB) {
        ...
        document.getElementById('li-modal-loading').style.display = 'flex';
        ...
        fetch(url, { credentials: 'same-origin' })
            .then(function (r) { return r.json(); })
            .then(function (data) {
                document.getElementById('li-modal-loading').style.display = 'none';
        ...
        modalOverlay.innerHTML =
            '<div class="li-modal">' +
                ...
                '<div class="li-modal-canvas-wrap">' +
                    '<div class="li-loading li-modal-loading" id="li-modal-loading" style="display:none;">' +
                        '<svg class="spin" width="28" height="28" viewBox="0 0 50 50">' +
                            '<circle cx="25" cy="25" r="20" stroke="#3CACA8" stroke-width="5" fill="none" stroke-linecap="round" stroke-dasharray="90 126" stroke-dashoffset="0"></circle>' +
                        '</svg>' +
                        '<span>Loading…</span>' +
                    '</div>' +
