...
function ExtractXML() {
    ...
    $.ajax({
        ...
        error: function(xhr) {
            document.getElementById("ExtractPending")?.remove();
            section.classList.add("status-panel--error", "status-panel--flat");
            const message = xhr.status === 403 ? "Insufficient permissions" : xhr.responseText;
            section.innerHTML += `<div class="status-summary"><span class="status-icon">&#9888;</span> ${message}</div>`;
            failExtraction();
        }
    });
