...
def launch_excel_export(export_type, object_id, xml_list, excel_format='simple'):
    task = generate_excel_file.delay(export_type, object_id, xml_list, excel_format)
    return task.id
...
def download_from_workspace(request):
    ...
    try:
        ...
        excel_format = data.get('format', 'simple')

        task_id = launch_excel_export('workspace', workspace_id, xml_list, excel_format)
...
def download_from_user_data(request):
    ...
    try:
        ...
        task_id = launch_excel_export('user', user_id, xml_list, excel_format)
        return JsonResponse({'task_id': task_id}, status=202)
...
def download_selected_data(request):
    ...
    try:
        ...
        task_id = launch_excel_export('selected', len(id_list), xml_list, excel_format)
        return JsonResponse({'task_id': task_id}, status=202)
...
def download_all_data(request):
    ...
    try:
        ...
        task_id = launch_excel_export('query', query_id, xml_list, excel_format)
        return JsonResponse({'task_id': task_id}, status=202)
