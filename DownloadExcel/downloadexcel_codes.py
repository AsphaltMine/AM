...
def grouped_column_order(rows):
    ...
    for row in rows:
        for key in row:
            ...
            if key not in columns_by_group[group]:
                columns_by_group[group].append(key)
    ordered = []
    for group in group_order:
        ordered.extend(columns_by_group[group])
    return ordered

def rows_from_xml(xml_contents):
    return [extract_paths_and_values(ET.fromstring(xml_content)) for xml_content in xml_contents]

def rows_by_test_type(xml_contents):
    grouped = defaultdict(list)
    for xml_content in xml_contents:
        root = ET.fromstring(xml_content)
        grouped[root.tag].append(extract_paths_and_values(root))
    return grouped

def build_ordered_dataframe(rows):
    df = pd.DataFrame(rows)
    df = df.dropna(axis=1, how='all')
    ordered_columns = [col for col in grouped_column_order(rows) if col in df.columns]
    return df[ordered_columns]

def build_simple_dataframe(xml_contents):
    return build_ordered_dataframe(rows_from_xml(xml_contents))

def main_simple(xml_contents, unique_suffix):
    grouped_rows = rows_by_test_type(xml_contents)
    output_path = os.path.join(settings.BASE_DIR, f"Download_Simple_{unique_suffix}.xlsx")
    with pd.ExcelWriter(output_path, engine='openpyxl') as writer:
        for test_type, rows in grouped_rows.items():
            df = build_ordered_dataframe(rows)
            df.to_excel(writer, sheet_name=test_type[:31], index=False)
    return output_path

def main_csv(xml_contents, unique_suffix):
    df = build_simple_dataframe(xml_contents)
    output_path = os.path.join(settings.BASE_DIR, f"Download_Simple_{unique_suffix}.csv")
