...
- Addition of a Download Data feature, offering multiple export formats (a simplified Excel file grouped by test, a flat CSV file, and the original AsphaltMine Excel template) for downloading search results.
- Implementation of self-service account registration with email verification. New accounts are granted read-only access by default, and an administrator can grant write access afterward.
