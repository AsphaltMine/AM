...
ALL_PARSED_RECORDS_CACHE_KEY = "live_insights:parsed_records:all"
ALL_PARSED_RECORDS_COMPUTED_AT_CACHE_KEY = "live_insights:parsed_records:all:computed_at"
ALL_PARSED_RECORDS_CACHE_TTL_SECONDS = 2 * 3600
...
ParsedRecord = namedtuple("ParsedRecord", ["test_key", "values", "sieve_points", "stiffness_moduli"])
...
def _iter_parsed_records(records):
    ...
    for record in records:
        ...
        yield ParsedRecord(
            ...
            stiffness_moduli=_collect_stiffness_moduli(root),
        )


def refresh_all_parsed_records_cache():
    """Recomputes the sitewide parsed dataset and stamps when. Called by the
    hourly beat task and by the manual refresh endpoint. Returns
    (parsed_records, computed_at)."""
    parsed_records = list(_iter_parsed_records(_resolve_records("all", [], None)))
    computed_at = timezone.now().isoformat()
    cache.set(ALL_PARSED_RECORDS_CACHE_KEY, parsed_records, timeout=ALL_PARSED_RECORDS_CACHE_TTL_SECONDS)
    cache.set(ALL_PARSED_RECORDS_COMPUTED_AT_CACHE_KEY, computed_at, timeout=ALL_PARSED_RECORDS_CACHE_TTL_SECONDS)
    return parsed_records, computed_at


def _get_all_parsed_records_cached():
    """Returns (parsed_records, computed_at), computing+caching once if
    nothing's cached yet (first request after a fresh deploy)."""
    parsed_records = cache.get(ALL_PARSED_RECORDS_CACHE_KEY)
    computed_at = cache.get(ALL_PARSED_RECORDS_COMPUTED_AT_CACHE_KEY)
    if parsed_records is None or computed_at is None:
        return refresh_all_parsed_records_cache()
    return parsed_records, computed_at


def _get_parsed_records(scope, workspace_ids, user):
    """Returns (parsed_records, computed_at). computed_at is None for the
    live (workspace) path -- it's always current, no staleness to report."""
    if scope == "all":
        return _get_all_parsed_records_cached()
    records = list(_iter_parsed_records(_resolve_records(scope, workspace_ids, user)))
    return records, None
...
def get_overview(scope, workspace_ids, user, mix_types=None, years=None, binder_grades=None):
    parsed_records, computed_at = _get_parsed_records(scope, workspace_ids, user)
...
def get_test_detail(test_key, scope, workspace_ids, user, mix_types=None, years=None, binder_grades=None):
    ...
    all_parsed, computed_at = _get_parsed_records(scope, workspace_ids, user)
    ...
    if test_key == "stiffness":
        moduli = [v for p in parsed_records for v in p.stiffness_moduli]
...
def _correlation_field_value(parsed, field_key, test_key):
    ...
    if field_key == "stiffness_modulus":
        moduli = parsed.stiffness_moduli
